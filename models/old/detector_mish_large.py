...
class Model(nn.Module):
    ...
    def __init__(self, d = 3, k = 10, emb_dims = 1024, dropout = 0.5):
        ...
        self.conv6 = nn.Sequential(nn.Conv2d(64,                    64,             kernel_size=1, bias=False), nn.BatchNorm2d(64),Mish())

        self.conv7 = nn.Sequential(nn.Conv2d(64*2,                  64,             kernel_size=1, bias=False), nn.BatchNorm2d(64),Mish())
        self.conv8 = nn.Sequential(nn.Conv2d(64,                    64,             kernel_size=1, bias=False), nn.BatchNorm2d(64),Mish())

        self.conv9 = nn.Sequential(nn.Conv1d(64*4,                  self.emb_dims,  kernel_size=1, bias=False), nn.BatchNorm1d(self.emb_dims),Mish())

        self.conv10 = nn.Sequential(nn.Conv1d(self.emb_dims+64*4,    512,            kernel_size=1, bias=False), nn.BatchNorm1d(512),Mish())
        self.conv11 = nn.Sequential(nn.Conv1d(512,                   256,            kernel_size=1, bias=False), nn.BatchNorm1d(256),Mish())

        self.conv12 = nn.Conv1d(256, 2, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = torch.cat((x1, x2, x3, x4), dim=1)          # (batch_size, 64*4, num_points)
        x = self.conv9(x)                               # (batch_size, 64*4, num_points) -> (batch_size, emb_dims, num_points)
        x = x.max(dim=-1, keepdim=True)[0]              # (batch_size, emb_dims, num_points) -> (batch_size, emb_dims, 1)

        x = x.repeat(1, 1, num_points)                  # (batch_size, 1024, num_points)
        x = torch.cat((x, x1, x2, x3, x4), dim=1)       # (batch_size, 1024+64*4, num_points)

        x = self.conv10(x)                               # (batch_size, 1024+64*4, num_points) -> (batch_size, 512, num_points)
        x = self.conv11(x)                               # (batch_size, 512, num_points) -> (batch_size, 256, num_points)
        x = self.conv12(x)                               # (batch_size, 256, num_points) -> (batch_size, 2, num_points)
