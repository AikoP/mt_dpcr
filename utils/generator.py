...
def readModels(model_paths, device = torch.device('cpu'), incrementKNNid = True):
    ...
    for (i, model_path) in enumerate(model_paths):
        ...
        if incrementKNNid:
            ...
            knns.append(utils.knn(model, 6).to('cpu'))
...
def getTrainingSet(
    ...
    train_knn_sizes = [knn.size(0) for knn in train_knns]
    train_knns = [knn + sum(train_knn_sizes[:i]) for (i, knn) in enumerate(train_knns)]
...
def getCorrectorSamples(model, sample_list, predictor, detector, corrector, max_pipeline_iterations):

    """
        model - (N x d) tensor
        sample_list - python list of a single hole growing process (first entry is the compolete model, last entry is the most 'eroded' model)
        ..
    """

    corrector_samples = []

    prev_train_data_length = 0
    start = time.time()

    # set the input (the most 'eroded' step)
    input = model[sample_list[-1][:,0]]

    max_iters = min(max_pipeline_iterations, len(sample_list))

    _, new_points_list = reconstructor.reconstruct(input, predictor, detector, corrector = corrector, max_iters = max_iters, t = 0)

    sample_input = input.detach().cpu().clone()
    sample_target = torch.zeros(sample_input.size())

    for new_points in new_points_list:

        # calculate for each predicted point the closest point from the complete model
        Dt_min = torch.min(utils.cdist(new_points.to(model.device), model), dim = 1)[1]
        ...
        sample_target = torch.cat([sample_target, model[Dt_min].to(new_points.device) - new_points])

        # the complete sample contains inputs and corresponding targets 
        corrector_samples.append(torch.cat([sample_input[:,:,None], sample_target[:,:,None]], dim=2))   # (N, d, 2)

    return corrector_samples
...
def getCorrectorTrainData(args):
    ...
    for (i, (model, model_knn)) in enumerate(zip(train_models, train_knns)):
        ...
        for j in range(args.train_size):
            # get list of hole growing steps (first element is the complete tensor)
            sample_list = getSampleList(model_knn, 1, hidden_range = hidden_range, iteration_range = iteration_range)
            train_samples += getCorrectorSamples(model.to(device), sample_list, predictor, detector, corrector, args.max_pipeline_iterations)
        
        print ("   ..done! (%.1fs)" % (time.time() - start))
        
    print ("\n#Train Samples: %d" % len(train_samples))
    ...
    train_bins = [bin.to(device) for bin in utils.getBins([sample.size(0) for sample in train_samples], b=50)]
    train_bins_sub = [[] for bin in train_bins]
    bin_stats = []
    ...
    for (i, bin) in enumerate(train_bins):
        ...
        size_list = [train_samples[sample_id].size(0) for sample_id in bin]
        ...
        train_bins_sub[i] = torch.zeros((bin.size(0), min_lim, train_samples[0].size(1), 2), dtype=torch.float, device=device)
        
        for (j, sample_id) in enumerate(bin):

            sample = train_samples[sample_id.item()]
    ...
    for (i, (model, model_knn)) in enumerate(zip(train_models, train_knns)):
        ...
        for j in range(args.test_size):
            # get list of hole growing steps (first element is the complete tensor)
            sample_list = getSampleList(model_knn, 1, hidden_range = hidden_range, iteration_range = iteration_range)
            test_samples += getCorrectorSamples(model.to(device), sample_list, predictor, detector, corrector, args.max_pipeline_iterations)
    ...
    for (i, (model, model_knn)) in enumerate(zip(val_models, val_knns)):
        
        print ("Processing model %d of %d.." % (i+1, len(val_models)))
        ...
        for j in range(args.val_size):
            # get list of hole growing steps (first element is the complete tensor)
            sample_list = getSampleList(model_knn, 1, hidden_range = hidden_range, iteration_range = iteration_range)
            val_samples += getCorrectorSamples(model.to(device), sample_list, predictor, detector, corrector, args.max_pipeline_iterations)
    ...
    data = {
        ...
        "val_samples": val_samples,
        'sample_stats': {
            'sample_sizes': [sample.size(0) for sample in train_samples]
        }
    }
...
if __name__ == '__main__':
    ...
    if args.type == 'corrector':
        ...
        with open(pathDict[args.dataset]['base'] + '/' + args.dataset + ('_train_data_corrector'), 'wb') as file:
